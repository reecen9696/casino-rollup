...
import { TestConfig, TestResult, TestMetrics, RequestResult, LiveMetrics, ChartDataPoint } from '../types/performance';
...
export const usePerformanceTest = () => {
  ...
  const calculateMetrics = useCallback((results: RequestResult[]): TestMetrics => {
    ...
    const headsResults = betsWithData.filter(r => r.betData!.result === true).length;
    const tailsResults = betsWithData.filter(r => r.betData!.result === false).length;
    const headsGuesses = betsWithData.filter(r => r.betData!.guess === true).length;
    const tailsGuesses = betsWithData.filter(r => r.betData!.guess === false).length;
    const correctGuesses = betsWithData.filter(r => r.betData!.won).length;
    const totalPayout = betsWithData.reduce((sum, r) => sum + (r.betData!.payout || 0), 0);
  ...
  const updateLiveMetrics = useCallback((results: RequestResult[], startTime: number) => {
    ...
    const currentLatency = recentResults.length > 0 
    ...
    setLiveMetrics({
      ...
      elapsedTime: elapsed,
      estimatedTimeRemaining: 0, // Will be calculated based on test type
    });
  ...
  const runBurstTest = useCallback(async (requestCount: number) => {
  ...
  const runSustainedTest = useCallback(async (targetRps: number, durationSeconds: number) => {
    ...
    try {
      ...
      const sendRequest = async () => {
        if (requestCount >= maxRequests || abortController.current?.signal.aborted) {
          return;
        }
