...
export class ApiClient {
  ...
  async placeBet(bet: BetRequest): Promise<{ response: BetResponse; latency: number }> {
    const startTime = performance.now();
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Bet failed: ${response.status} ${response.statusText}`);
      }
      ...
      throw new Error(`Bet error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async getRecentBets(): Promise<{ bets: any[]; latency: number }> {
    ...
    try {
      ...
      throw new Error(`Get bets error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
